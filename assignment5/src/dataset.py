...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        sent = self.data[idx]
        sent_new_length = np.random.randint(low=4, high=int(self.block_size * 7 / 8) + 1)
        sent_trunc = sent[:sent_new_length]
        if len(sent_trunc) < 4:
            sent_trunc += ' ' * (4 - len(sent_trunc))
        masked_content_entry = np.random.randint(low=1, high=len(sent_trunc) - 1)
        masked_content_length = np.random.randint(low=1, high=len(sent_trunc[masked_content_entry:]))
        prefix = sent_trunc[:masked_content_entry]
        masked_content = sent_trunc[masked_content_entry:masked_content_entry + masked_content_length]
        suffix = sent_trunc[masked_content_entry + masked_content_length:]
        out = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content
        out_pad = self.PAD_CHAR * (self.block_size - len(out))
        out += out_pad
        x = out[:-1]
        y = out[1:]
        x = torch.tensor([self.stoi[c] for c in x], dtype=torch.long)
