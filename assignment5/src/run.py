...
if __name__ == '__main__':
    ...
    mconf = model.GPTConfig(pretrain_dataset.vocab_size, pretrain_dataset.block_size,
        n_layer=4, n_head=8, n_embd=256, synth_attn=False)
    ...
    if args.variant == 'vanilla':
        ...
        mconf_synth = model.GPTConfig(pretrain_dataset.vocab_size, pretrain_dataset.block_size,
                                n_layer=4, n_head=8, n_embd=256, synth_attn=True)
        model = model.GPT(mconf_synth).to(device)
    ...
    if args.function == 'pretrain':
        ...
        if args.reading_params_path is not None:
            ...
            tconf = trainer.TrainerConfig(max_epochs=75, batch_size=256, learning_rate=6e-4,
                                          ...
                                          final_tokens=200 * len(pretrain_dataset) * block_size,
                                          num_workers=4)
        name_dataset = dataset.NameDataset(pretrain_dataset, open('birth_places_train.tsv', encoding='utf8').read())
