...
class SynthesizerAttention(nn.Module):
    ...
    def forward(self, x, layer_past=None):
        ...
        # Only the V vector is projected in this type of attention
        v = self.value(x).view(B, T, self.n_head, C // self.n_head).transpose(1, 2)

        # Synthesize the keys and queries quadradic dot product:
        l1 = F.relu(self.w1(x)).view(B, T, self.n_head, C // self.n_head).transpose(1, 2) # (B, nh, T, hs)
        att = l1 @ self.w2[:, :T] + self.b2[:T] # (B, nh, T, T)

        # Continue like regular multi-head
        att = att.masked_fill(self.mask[:,:,:T,:T] == 0, -1e10) # todo: just use float('-inf') instead?
        att = F.softmax(att, dim=-1)
        att = self.attn_drop(att)
        y = att @ v # (B, nh, T, T) x (B, nh, T, hs) -> (B, nh, T, hs)
