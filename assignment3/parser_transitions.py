...
class PartialParse(object):
    def __init__(self, sentence):
        ...
        self.stack = ['ROOT']
        self.buffer = [x for x in sentence]
        self.dependencies = []
    ...
    def parse_step(self, transition):
        ...
        if transition == 'S':
            self.stack.append(self.buffer.pop(0))  # Might be more efficient with deque and leftpop
        elif transition == 'LA':
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) > 0:
        for batch_idx in range(len(unfinished_parses) // batch_size + 1):
            unfinished_parses_batch = unfinished_parses[batch_size*batch_idx:batch_size*(batch_idx+1)]
            transitions = model.predict(unfinished_parses_batch)
            for partial_parse, transition in zip(unfinished_parses_batch,transitions):
