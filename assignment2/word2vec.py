...
def naiveSoftmaxLossAndGradient(
    ...
    gradCenterVec = -outsideVectors[outsideWordIdx] + probs.dot(outsideVectors)
    gradOutsideVecs = probs.reshape(-1,1).dot(centerWordVec.reshape(1,-1))
...
def negSamplingLossAndGradient(
    ...
    uo_T_vc = outsideVectors[outsideWordIdx].dot(centerWordVec)
    uk_T_vc = outsideVectors[negSampleWordIndices].dot(centerWordVec.T)
    loss = -np.log(sigmoid(uo_T_vc)) - np.sum(np.log(sigmoid(-uk_T_vc)))
    gradCenterVec = (sigmoid(uo_T_vc)-1)*outsideVectors[outsideWordIdx] + np.sum((1-sigmoid(-uk_T_vc)).reshape(-1,1)*outsideVectors[negSampleWordIndices], axis=0)
    gradOutsideVecs = np.zeros_like(outsideVectors)
    gradOutsideVecs[outsideWordIdx] = (sigmoid(uo_T_vc)-1)*centerWordVec
    np.add.at(gradOutsideVecs, np.array(negSampleWordIndices), ((1-sigmoid(-uk_T_vc)).reshape(-1,1)*centerWordVec))
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    vc = centerWordVectors[vc_idx]
    outer_words_idx_list = [word2Ind[w] for w in outsideWords]

    for wo_idx in outer_words_idx_list:
        curr_loss, curr_gradCenterVecs, curr_gradOutsideVectors = word2vecLossAndGradient(vc, wo_idx, outsideVectors, dataset)
        loss += curr_loss
        gradCenterVecs[vc_idx] += curr_gradCenterVecs
        gradOutsideVectors += curr_gradOutsideVectors
