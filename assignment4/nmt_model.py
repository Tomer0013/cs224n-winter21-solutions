...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        source_embeddings = torch.nn.utils.rnn.pack_padded_sequence(source_embeddings, source_lengths)
        enc_out, (enc_h, enc_c) = self.encoder(source_embeddings)
        enc_out, _ = torch.nn.utils.rnn.pad_packed_sequence(enc_out)
        enc_hiddens = enc_out.permute(1, 0, 2)
        enc_h = enc_h.permute(1, 0, 2).flatten(start_dim=-2, end_dim=-1)
        enc_h = self.h_projection(enc_h)
        enc_c = enc_c.permute(1, 0, 2).flatten(start_dim=-2, end_dim=-1)
        enc_c = self.h_projection(enc_c)  # (b, h)
        dec_init_state = (enc_h, enc_c)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        enc_hidden_proj = self.att_projection(enc_hiddens)  # (b, src_len, h)
        y = self.model_embeddings.target(target_padded)  # (tgt_len, b, emb_dim)
        y_split = y.split(1)
        combined_outputs = []
        for y_t in y_split:
            y_t = y_t.squeeze()
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t = torch.bmm(dec_hidden.unsqueeze(1), enc_hiddens_proj.permute(0, 2, 1)).squeeze()
